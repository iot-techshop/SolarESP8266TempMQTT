...
const char* ssid = "YourWiFiSSID";   //Use Your Wifi SSID  Note: ESP's only work on 2,4GHz wifi, 
//be sure to have one active on your Router
const char* password = "YourWiFiPassword"; //Use your WiFi Password
const char* mqtt_server = "broker.hivemq.com";  //You can use iot.cuspix.com or other mqt server URL
const char* mqtt_topic = "YourName/Solar/temp"; //Set MQTT Topic
...
void setup() {
  ...
  //delay(5000);
  pinMode(BUILTIN_LED, INPUT);     // Initialize the BUILTIN_LED pin as an input to conserve power
...
void loop() {
  ...
  volts = (analogRead(A0) * vCal);
  vBat = String(volts);
  
//Adjust your values in this section bassed on your own experimentation with battery life and solar recharge

  if (volts > 4.20) {
    rInt = 60;
  }
  if ((volts > 4.0) && (volts <= 4.20)) {
    rInt = 300;
  }
  if ((volts > 3.8) && (volts <= 4.00)) {
    rInt = 600;
  }
  if ((volts > 3.7) && (volts <= 3.80)) {
    rInt = 900;
  }
  if (volts <= 3.70) {
    rInt = 3600;
  ...
  myString = "{\"ID\":\"" + devID + "\",\"time\":\"" + myDateTime + "\",\"" + "devTemp" + "\":\"" + myTemp + "\",\"" + "vBat" + "\":\"" + vBat + "\",\"" + "rInt" + "\":\"" + rInt + "\"}";
